...
# search directories under HST_DIR to get the cycle list
    command='pwd'
    current_path=subprocess.getoutput(command)
    flag=1
...
# when a visit is selected, get the imaging data from the biz file
    ...
    fname=subprocess.getstatusoutput(command)[1]
...
# show the image of a selected system
    ...
    mouse_mode_button=tkinter.ttk.Combobox(frame_popup, textvariable=mouse_mode_text, state='readonly')
    ...
    mask_mode_button=tkinter.ttk.Combobox(frame_popup, textvariable=mask_mode_text, state='readonly')
    ...
    brush_text.set('1')
    brush_button=tkinter.ttk.Combobox(frame_popup, textvariable=brush_text, state='readonly')
...
#	blank_label=Label(frame_popup, width=12, text=' ').grid(row=0, column=10)
    ...
    NavigationToolbar2Tk(canvas, popup)
...
def run_phot_fit_mcmc():
    ...
    res=[(v[1], v[2]-v[1], v[1]-v[0]) for v in zip(*np.percentile(samples, [16, 50, 84], axis=0))]	
    p_mcmc=[]
    for i in range(ndim):
...
def show_phot_fit():
    ...
    NavigationToolbar2Tk(canvas_phot, popup)	
...
#	popup.protocol("WM_DELETE_WINDOW", lambda: popup.destroy())
    ...
    NavigationToolbar2Tk(canvas_fit, popup)	
    ...
    save_fit_button=tkinter.ttk.Combobox(frame, textvariable=save_fit_button_text)
