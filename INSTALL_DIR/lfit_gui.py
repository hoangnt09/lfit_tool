...
class Application:
	...
	var_cycle=IntVar()
	cycle_menu=OptionMenu(*(target_panel, var_cycle)+tuple(cycle_list))
	...
	var_program=IntVar()
	program_menu=OptionMenu(*(target_panel, var_program)+tuple(program_list))
	...
	target_tree=tkinter.ttk.Treeview(target_panel, columns=targetColumns)
	for i in range(2):
	...
	target_scrollbar=tkinter.ttk.Scrollbar(target_panel, orient="vertical", style='blue.Vertical.TScrollbar', command=target_tree.yview)
	...
	add_lens_model_button=tkinter.ttk.Combobox(control_panel, textvariable=add_lens_model_text, state='readonly')
	...
	add_phot_model_button=tkinter.ttk.Combobox(control_panel, textvariable=add_phot_model_text, state='readonly')
	...
	add_source_model_button=tkinter.ttk.Combobox(control_panel, textvariable=add_source_model_text, state='readonly')
